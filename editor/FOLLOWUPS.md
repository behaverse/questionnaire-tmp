...
## ED-K2 (Library Browser — edit + contribution) — deferred (final-review triage, 2026-06-21)
ED-K2 added structural editing to the Library browser: a local edit session (autosaved to IndexedDB `library-session`, restored on reopen), `EntityInspector` refactored to controlled + tabbed (Inspect | Edit), per-locale status + lock-on-complete (disabled `<fieldset>`), and full-body contribution export of edited entities. Reuses the standalone editors (not the store-coupled ItemEditor). Final review APPROVE-WITH-FOLLOWUPS (no Critical/Important). Per-task fixes already applied: PromptEditor scope-creep reverted; fetch-effect render-loop dep removed; originals seeded on restore; saving-indicator visibility. Deferred minors:
- ed-k2-1: the inspector's "Entity status" select duplicates each editor's own per-locale status field (e.g. PromptEditor's "Prompt text status") in the Edit tab — redundant/confusing. The editor's inner status gets disabled+trapped inside the locked fieldset; only the outer "Entity status" select can unlock. Fix: give the editors a `showStatus={false}` (or `hideStatus`) prop and hide their inner status in the browser Edit-tab context, leaving the inspector's as the canonical control.
- ed-k2-2: `edited` recomputes `JSON.stringify` over all session bodies on every render — fine at the capped scale; memoize if sessions grow large.
- ed-k2-3: lock semantics are per (entity, edit-locale): structural fields (shared across locales) are locked when the CURRENT locale is complete — switch locale or set draft to edit structural. Acceptable v1 simplification; revisit if confusing.
- NEXT: ED-K3 (Translate tab reusing fields/apply/translateText + batch translate; retire `TranslationWorkbench.tsx` + its start-screen card + the old `translations.<lang>.json` export).

## ED-K3 (Library Browser — Translate tab + batch + retire workbench) — deferred (final-review triage, 2026-06-21)
