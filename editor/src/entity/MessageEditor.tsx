...
export function MessageEditor({ message, locale, primaryLocale, showStatus = true, onChange }: { message: MessageBody; locale: string; primaryLocale?: string; showStatus?: boolean; onChange: (m: MessageBody) => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <ContentTextEditor content={message.content} locale={locale} label="Message text" primaryLocale={primaryLocale}
                         showStatus={showStatus} onChange={(c) => onChange({ ...message, content: c })} />
    </div>
  )
}
