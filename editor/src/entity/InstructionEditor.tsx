...
export function InstructionEditor({ instruction, locale, primaryLocale, showStatus = true, onChange }: { instruction: InstructionBody; locale: string; primaryLocale?: string; showStatus?: boolean; onChange: (i: InstructionBody) => void }) {
  ...
  return (
    <div className="space-y-2">
      <ContentTextEditor content={instruction.content} locale={locale} label="Instruction text" primaryLocale={primaryLocale}
                         showStatus={showStatus} onChange={(c) => onChange({ ...instruction, content: c })} />
