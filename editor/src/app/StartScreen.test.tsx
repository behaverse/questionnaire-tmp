...
test('New creates an empty questionnaire', async () => {
  const onNew = vi.fn()
  render(<StartScreen onNew={onNew} onOpenFile={vi.fn()} onOpenLibrary={vi.fn()} onLoadSample={noop} onBrowseLibrary={noop} onTranslate={noop} onLoadPhq9={noop} onBrowseEntities={noop} />)
  await userEvent.click(screen.getByRole('button', { name: /new questionnaire/i }))
  expect(onNew).toHaveBeenCalled()
})

test('shows the three entry points', () => {
  render(<StartScreen onNew={vi.fn()} onOpenFile={vi.fn()} onOpenLibrary={vi.fn()} onLoadSample={noop} onBrowseLibrary={noop} onTranslate={noop} onLoadPhq9={noop} onBrowseEntities={noop} />)
...
describe('StartScreen Load a sample', () => {
  it('renders a Load a sample action and calls onLoadSample', () => {
    ...
    render(<StartScreen onNew={noop} onOpenFile={noop} onOpenLibrary={noop} onLoadSample={onLoadSample} onBrowseLibrary={noop} onTranslate={noop} onLoadPhq9={noop} onBrowseEntities={noop} />)
  ...
  it('the "Load PHQ-9 sample" card calls onLoadPhq9', () => {
    ...
    render(
      <StartScreen onNew={noop} onOpenFile={noop} onOpenLibrary={noop} onLoadSample={noop}
                   onBrowseLibrary={noop} onTranslate={noop} onLoadPhq9={onLoadPhq9}
...
describe('StartScreen Library entities', () => {
  it('the "Library entities" card calls onBrowseEntities', () => {
    ...
    render(<StartScreen onNew={noop} onOpenFile={noop} onOpenLibrary={noop} onLoadSample={noop}
                        onBrowseLibrary={noop} onTranslate={noop} onLoadPhq9={noop}
