...
- **Piping transform** — `editor/src/logic/piping.ts` (pure, port of the viewer): `pipedText(fieldPath, original, rules, ev, bindings)` (first matching piping rule whose `field_path === fieldPath`, valid [`ev.check === null`] + true [`ev.condition`] → `bindings.var(source)` [array `', '`-joined], nullish/no-match → original) and `applyPiping(page, rules, ev, bindings, locale)` → new `RuntimePage` with each top-level item's `question.prompt.content[locale].text` rewritten where a rule fires (single active locale, mirroring the viewer's App rewrite).
...
- **Live same-page piping preview** — `PreviewPane` applies `applyPiping` to the **full pages first (stable original element indices)**, THEN `filterPageVisible` removes hidden elements. (Pipe-then-filter, NOT filter-then-pipe: the `field_path` index `pages.{id}.elements.{idx}.prompt` is authored against the unfiltered element position, so piping must run before visibility filtering shifts indices. Hidden elements are removed afterward → identical result to the viewer, which pipes using a stable index map.) Answering the source re-renders → re-pipes live. Malformed condition → original (false-safe).
...
- **`editor/src/logic/piping.ts`** (pure, port of `web-viewer/src/logic/piping.ts` + the App rewrite):
  - `pipedText(fieldPath, original, rules, ev, bindings)`: for each `type:'piping'` rule with `action.field_path === fieldPath`, valid (`ev.check(cond) === null`) + true (`ev.condition`): `const v = bindings.var(String(action.source ?? '')); if (v == null) return original; return Array.isArray(v) ? v.join(', ') : String(v)`. No firing rule → `original`. (Mirrors `web-viewer/src/logic/piping.ts`; the `ev.check` guard is the editor's addition since it doesn't pre-compile rules — false-safe to original on malformed.)
  - `applyPiping(page, rules, ev, bindings, locale)`: new page; for each top-level element whose `question.prompt.content[locale].text` is a string, set it to `pipedText('pages.'+page.id+'.elements.'+i+'.prompt', text, rules, ev, bindings)` (only the active `locale`, exactly like the viewer's App). Shallow-clone only the element/question/prompt/content/[locale] path being written; if `piped === text` return the element unchanged (no needless clone). Non-item elements untouched. The index `i` is the position in `page.elements` of the page passed in — call `applyPiping` on the FULL (unfiltered) pages so `i` matches the authored `field_path` index.
...
- **`editor/src/preview/PreviewPane.tsx`** — pipe BEFORE filtering: `const pipedPages = evaluator ? pages.map((p) => applyPiping(p, model.logic ?? [], evaluator, bindings, locale)) : pages` then `const visiblePages = evaluator ? pipedPages.map((p) => filterPageVisible(p, evaluator, bindings, model.logic ?? [])) : pipedPages` (render `visiblePages`). Pipe-then-filter keeps element indices stable for `field_path` matching.
...
**Preview:** `pages → applyPiping (full pages, stable indices) → filterPageVisible → render`. `answers` is editor state, so answering the source re-renders → re-pipes. Pipe-then-filter keeps `field_path` element indices stable (a hidden element is removed after, so it's never shown anyway — same result as the viewer). `scope:'all'` pipes cross-page (global answers); `scope:'page'` only same-page sources are answerable — both correct.
...
- `piping` — `pipedText` scalar/array/no-match/false/malformed; `applyPiping` rewrites the matched element's active-locale prompt, leaves others, no mutation, uses the full-page index (stable), returns element unchanged when piped===original.
...
- `PreviewPane` — a piping rule rewrites the target prompt after the source is answered; pipe-then-filter so a visibility-hidden element doesn't break the index of a later piped element.
...
2. Preview pipes live (source answer, array-joined) once the source is answered; pipe-then-filter (stable indices); malformed → original.
