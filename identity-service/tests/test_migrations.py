...
def test_baseline_recorded_and_rerun_is_noop(conn):
    # conftest applied the baseline at session setup → it's recorded, and re-running adds nothing
    applied = {r[0] for r in conn.execute("SELECT version FROM schema_migrations").fetchall()}
    assert "001_baseline.sql" in applied
...
def test_new_migration_applies_once_and_can_alter(conn, tmp_path):
    ...
    assert migrate.apply_migrations(conn, migrations_dir=d) == ["090_demo.sql"]
    ...
    conn.execute("DELETE FROM schema_migrations WHERE version='090_demo.sql'")
    conn.commit()
