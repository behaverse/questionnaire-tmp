...
- ~~**Dedicated `REPLAY_SIGNING_SECRET` + link revocation.**~~ **DONE (2026-07-03)** — replay tokens now sign with a dedicated `REPLAY_SIGNING_SECRET` (falls back to `INVITE_SIGNING_SECRET` when unset), so rotating it invalidates every outstanding replay link; new researcher-gated `POST /v1/deployments/{id}/sessions/{sid}/replay-link/revoke` + the participant-app `/studies` "Revoke links" button let a researcher kill all links for one session without a full rotation — `GET /v1/replay?token=` returns `401` for any token minted before the revoke.
- ~~**Incremental live-follow of an in-progress session.**~~ **DONE (2026-07-03)** — `GET /v1/replay?token=` still returns a point-in-time snapshot, but a new player follow mode (`&follow=1`, driven by the `/studies` "Watch live" button) polls it every ~4s and keeps the view pinned to the latest event, stopping automatically when the session ends; see `web-viewer/docs/replay.md` + `web-viewer/FOLLOWUPS.md`.
