...
> The public, **read-only catalogue** SPA for the Library: search → view metadata + items → download canonical Schema-2 JSON, plus a **"Try it"** demo that launches the live player render-only. SPA + the Library API are served same-origin on Vercel, reading Supabase.
...
- **Detail** (`/q/:id/:version`, `src/routes/DetailPage.tsx`) — metadata, items, scores, version list, **Download JSON**, and **Try it**.
...
- All API access goes through `src/api/client.ts` (base = `VITE_API_BASE_URL`, empty = same-origin in prod) + `src/api/queries.ts` (TanStack Query). Render projection in `src/definition/renderModel.ts`.
...
- Tests: `npm test` (vitest + RTL; broad coverage across `api/`, `catalogue/`, `detail/`, `lib/`, `definition/`). Build: `npm run build` (`tsc -b` + test tsconfig + `vite build`).
...
- **No content re-import** — canonical content is live on Supabase (212 Qs); never re-run the importer to "refresh" this UI.
