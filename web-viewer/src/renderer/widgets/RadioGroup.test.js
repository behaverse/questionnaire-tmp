...
test('renders one radio per choice inside a named group; selecting reports the value', async () => {
    ...
    render(_jsx(RadioGroup, { name: "it_1", label: "Little interest", choices: choices, value: null, onChange: onChange }));
...
test('shows letter hints and selects via keyboard letter when keyHints is on', async () => {
    const onChange = vi.fn();
    render(_jsx(RadioGroup, { name: "it_1", label: "L", choices: choices, value: null, onChange: onChange, keyHints: true }));
...
test('letter keys ignored without keyHints and when typing in a text input', async () => {
    const onChange = vi.fn();
    render(_jsxs(_Fragment, { children: [_jsx("input", { type: "text", "aria-label": "other" }), _jsx(RadioGroup, { name: "it_1", label: "L", choices: choices, value: null, onChange: onChange, keyHints: true })] }));
    await userEvent.type(screen.getByLabelText('other'), 'a');
    expect(onChange).not.toHaveBeenCalled();
});
test('selected card reflects value; no axe violations', async () => {
    const { container } = render(_jsx(RadioGroup, { name: "it_1", label: "L", choices: choices, value: 1, onChange: vi.fn() }));
