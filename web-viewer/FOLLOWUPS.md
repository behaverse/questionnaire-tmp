...
- ~~**#7 Replay — RP1 (offline, file-based).**~~ **DONE (2026-06-30)** — `?replay=<src>` mounts
  ...
  - ~~**RP3 — live `?replay=<deployment>/<session>` loader.**~~ **DONE (2026-07-02)** — verified:
    ...
    **Remaining RP3 follow-ons** (below): ~~a "copy replay link" UI, a researcher session-list
    ...
    + `viewer-service/FOLLOWUPS.md`. ~~Still remaining: live-follow.~~ **DONE (2026-07-03)** —
    `&follow=1` (the `/studies` "Watch live" button) polls `GET /v1/replay?token=` every ~4s and keeps
    the view pinned to the latest event, stopping when the session ends; see `web-viewer/docs/replay.md`.
  ...
  - ~~**Multi-select (checkbox) answers are not reconstructed in RP1**~~ **DONE (2026-07-02)** — replay
    now reconstructs checkbox state by folding the ordered `bdm:selected`/`bdm:deselected` events (each
    carrying `bdm:option_index`) into `selectedIndices`; forward-only, sessions recorded before this
    lack `bdm:option_index` and replay blank. See `src/replay/reconstruct.ts` +
    `tools/respondent-bot/tests/e2e/replay.spec.ts` (fixture `replay-bundle-multiselect.json`).
