...
- **Per-question QA comments (owner #5/#6):** opt-in via `runtime.style.x_comments: true`. A floating icon (bottom-right) on each step opens a modal (`src/app/chrome/CommentWidget.tsx`): free-text comment + optional 1–5 star rating, POSTed to `POST /v1/sessions/{id}/comments` via `submitComment` (best-effort, non-blocking; stored out-of-band from Schema-5 responses). Researchers read via `GET /v1/deployments/{id}/comments` (JSON) or `…/comments.csv`. Authored at the questionnaire level in the Editor (Presentation section). Try it locally with `?fixture=comments`. It's a **comment**, not "feedback" (owner term).
...
- **Replay (RP1–RP3, #7 — DONE):** `?replay=<bundle_url>` mounts `ReplayApp` (no session) — fetches a bundle `{runtime, statements, mouse?}` (e.g. the respondent-bot's `trace.json`, or a VS-minted `replay-link`), reconstructs the run from `bdm:` statements (`src/replay/reconstruct.ts`) — including multi-select (checkbox) answers via the `bdm:option_index` result-extension — and plays it back read-only through `StepRenderer` with a timeline scrubber, play/pause, speed (0.5/1/2/4×), and a mouse-cursor overlay (`src/replay/cursor.ts`). Normal run path is unchanged. Researcher-gated live loading (RP2: signed `replay-link` mint + revoke) and `?follow=1` live-follow (polls the bundle every ~4s, pinned to the latest event, stops on end/failure) both shipped; researcher-facing replay links + "Watch live" live in `participant-app`'s `/studies` surface. See [docs/replay.md](docs/replay.md) for the full contract.
- **Mouse capture (SP2 — mouse-tracking track complete):** when the deployment mint returns `channels.mouse: true`, the player captures the participant's pointer as Schema-4b `{t,x,y,button_state}` samples (default 60 Hz via a `mousemove` throttle of ≥16.7 ms; mousedown/mouseup always captured immediately; rate overridable via `?mouse_hz=<n>`). The recording is uploaded once at finish via the submission queue's `'recordings'` kind → `POST /v1/sessions/{id}/recordings` (SP3 endpoint); a best-effort `pagehide` stop-and-enqueue salvages an abandoned run. Capture is bracketed by `bdm:recording_started` (extensions: `recording_modality`, `sample_rate`, `recording_scope`) at run-start and `bdm:recording_ended` (`recording_url` → `/v1/deployments/{id}/recordings`, `sample_count`) at finish. Capture is skipped on fixture, preview, and ephemeral runs — only real `?deployment=` runs with `channels.mouse` on. New code: `src/app/mouseCapture.ts`, `transport.ts` (`SubmissionKind += 'recordings'`), `events.ts` (two recording builders), wiring in `App.tsx`/`bootstrap.ts`. SP1 (respondent-bot mouse path) and SP3 (VS recording store + channels mint) already merged — the player-side track is functionally complete, pending live end-to-end verification.
