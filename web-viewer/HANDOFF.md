...
- **Offline replay mode (RP1, #7):** `?replay=<src>` mounts `ReplayApp` (no session) — fetches a bundle `{runtime, statements, mouse?}` (e.g. the respondent-bot's `trace.json`), reconstructs the run from `bdm:` statements (`src/replay/reconstruct.ts`), and plays it back read-only through `StepRenderer` with a timeline scrubber, play/pause, speed (0.5/1/2/4×), and a mouse-cursor overlay (`src/replay/cursor.ts`). Normal run path is unchanged. Live researcher-gated loading is RP2 (VS event-export reads) + RP3 (`?replay=<deployment>/<session>` loader).
