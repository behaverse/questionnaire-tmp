...
The Web Viewer evaluates all four declared logic actions — **skip**, **visibility**, **piping**, and **branch** — together with per-question and cross-question `show_if` predicates, using the OD-11 expression evaluator (see [15_expression_language.md](15_expression_language.md)). At answer-commit the viewer computes `reversed_value` (producing the post-reversal `scored_value`) and, for Solution-bearing items, the per-item `correct` flag. Navigation follows a graph walk: skip and branch actions jump forward over intervening steps, hidden steps are silently bypassed, and Back retraces the participant's visited path rather than the raw step sequence. Per OD-16 (resolved 2026-06-02; see [05b_scoring.md](05b_scoring.md)), `score(id)` resolves via the pinned Scorer implementation embedded in Schema 3 at session-mint (per OD-18, [05d_runtime.md](05d_runtime.md)); branching-required scores always evaluate at page-submit, while display scores are `show_score`-gated, per the two-trigger model in 05b_scoring.md §4.4.
